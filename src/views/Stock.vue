			...
			<div class="flex justify-between items-center mb-4">
				...
				<div class="flex items-center space-x-2">
					...
					<button
						...
						class="add-btn flex items-center justify-center w-10 h-10 text-white rounded-lg"
					>
						<img
		...
		const products = ref([
			{
				...
				buyPrice: 20000,
				sellPrice: 25000,
				image: require("@/assets/Product.jpeg"),
			},
			{
				id: 1,
				name: "Nuget Kanzler",
				...
				if (response.data && Array.isArray(response.data.data)) {
					...
					const transformed = rawProducts.map((product) => {
						const fullImageUrl = product.image
							? `https://nurulfrozen.dgeo.id${product.image}`
							: null;
